...
function analyzeLabExam(exam: LabExamEntry) {
  ...
  if (exam.ht) {
    if (ht < 21) pushItem('HT', 'CRITICO — anemia severa', red)
    else if (ht < 36) pushItem('HT', 'Baixo', yellow)
    else if (ht > 55) pushItem('HT', 'Hemoconcentracao', red)
    else if (ht > 50) pushItem('HT', 'Elevado', yellow)
    else pushItem('HT', 'Normal', green)
  }
  const inr = parseNumber(exam.inr)
  if (exam.inr) {
    if (inr > 4) pushItem('INR', 'CRITICO — hemorragia', red)
    else if (inr > 2) pushItem('INR', 'Moderado — risco sangramento', orange)
  ...
  if (exam.k) {
    if (potassium > 6.5) pushItem('K+', 'EMERGENCIA — arritmia fatal', red)
    else if (potassium > 5.5) pushItem('K+', 'Hipercalemia moderada', orange)
    else if (potassium > 5.0) pushItem('K+', 'Hipercalemia leve', yellow)
    else if (potassium < 2.5) pushItem('K+', 'CRITICO — parada cardiaca', red)
    else if (potassium < 3.0) pushItem('K+', 'Hipocalemia grave', orange)
    else if (potassium < 3.5) pushItem('K+', 'Hipocalemia leve', yellow)
    else pushItem('K+', 'Normal', green)
  }
  const sodium = parseNumber(exam.na)
  if (exam.na) {
    if (sodium < 125) pushItem('Na+', 'CRITICO — edema cerebral', red)
    else if (sodium < 135) pushItem('Na+', 'Hiponatremia', yellow)
    else if (sodium > 155) pushItem('Na+', 'CRITICO — desidratacao celular', red)
  ...
  if (exam.lac) {
    if (lactate >= 4) pushItem('Lactato', 'CRITICO — acidose lactica', red)
    else if (lactate >= 2) pushItem('Lactato', 'Hiperlactatemia', yellow)
    else pushItem('Lactato', 'Normal', green)
  }
  ...
  if (exam.bt) {
    if (bt > 2.5) pushItem('BT', 'Ictericia clinica', red)
    else if (bt > 1.2) pushItem('BT', 'Elevada leve', yellow)
    else pushItem('BT', 'Normal', green)
  }
  const alb = parseNumber(exam.alb)
  if (exam.alb) {
    if (alb < 2.5) pushItem('Alb', 'CRITICO — anasarca', red)
    else if (alb < 3.5) pushItem('Alb', 'Hipoalbuminemia', yellow)
    else pushItem('Alb', 'Normal', green)
  }
  const tgo = parseNumber(exam.tgo)
  if (exam.tgo) {
    if (tgo > 1000) pushItem('TGO', 'CRITICO — hepatite fulminante', red)
    else if (tgo > 100) pushItem('TGO', 'Injuria hepatica moderada', orange)
    else if (tgo > 40) pushItem('TGO', 'Elevada leve', yellow)
    else pushItem('TGO', 'Normal', green)
  }
  const tgp = parseNumber(exam.tgp)
  if (exam.tgp) {
    if (tgp > 1000) pushItem('TGP', 'CRITICO — hepatite fulminante', red)
    else if (tgp > 100) pushItem('TGP', 'Injuria hepatica moderada', orange)
    else if (tgp > 41) pushItem('TGP', 'Elevada leve', yellow)
    else pushItem('TGP', 'Normal', green)
  }

  // ══════════════════════════════════════════════
  // ── Alertas clinicos individuais ──
  // ══════════════════════════════════════════════
...
function StatusChoice({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="rounded-full border px-2 py-1 text-[8px] font-semibold uppercase tracking-[0.14em] transition-all"
      style={{
...
function ProtoSection({ title, color }: { title: string; color?: string }) {
  return (
    <p className="mb-1 mt-2 border-b border-white/8 pb-0.5 text-[8px] font-bold" style={{ color: color || 'rgba(255,255,255,0.86)' }}>
      {title}
    </p>
  )
}
...
function ProtoNote({ text }: { text: string }) {
  return <p className="py-0.5 text-[8px] leading-snug text-white/50">{text}</p>
}

function ProtoBox({ text }: { text: string }) {
  return (
    <div className="my-1 rounded-[0.5rem] border border-white/8 bg-white/3 px-2 py-1 text-[8px] leading-relaxed text-white/50">
      {text}
    </div>
  )
}
...
export function ProntuarioSystemPanel() {
  ...
  function createWorkspace(name: string) {
    if (!name.trim() || workspacesRef.current.length >= 6) return
    const newWs: Workspace = { id: generateId(), name: name.trim(), records: [], archive: [] }
    const updated = [...workspacesRef.current, newWs]
  ...
  function renameWorkspace(id: string, name: string) {
    if (!name.trim()) return
    const updated = workspacesRef.current.map(w => w.id === id ? { ...w, name: name.trim() } : w)
    workspacesRef.current = updated
    setWorkspaces(updated)
    localStorage.setItem(STORAGE_KEYS.workspaces, JSON.stringify(updated))
    setShowWsModal(false)
  ...
  function deleteWorkspace(id: string) {
    ...
    localStorage.setItem(STORAGE_KEYS.workspaces, JSON.stringify(updated))
    setShowWsModal(false)
    if (activeWsIdRef.current === id) switchWorkspace(updated[0].id)
  }
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // Step 1: localStorage workspaces (novo formato)
      try {
        ...
        if (storedWs) {
          ...
          // Migração legacy: sea-icu-records → primeiro setor "UTI"
          const legRec = localStorage.getItem(STORAGE_KEYS.records)
          const legArc = localStorage.getItem(STORAGE_KEYS.archive)
          const legRecords: ICURecord[] = legRec ? (JSON.parse(legRec) as Array<Partial<ICURecord>>).map(r => normalizeRecord(r)) : []
          const legArchive: ICURecord[] = legArc ? (JSON.parse(legArc) as Array<Partial<ICURecord>>).map(r => normalizeRecord(r)) : []
      ...
      // Step 2: Supabase — só substitui se for mais recente ou local vazio
      if (supabase) {
        try {
          ...
          if (data?.records) {
            ...
            const localIsEmpty = localWs.every(w => w.records.length === 0)
            if (localIsEmpty || supabaseUpdatedAt > localUpdatedAt) {
              ...
              if (raw && typeof raw === 'object' && !Array.isArray(raw) && (raw as Record<string, unknown>).__sea_v2) {
                ...
                localWs = v2.workspaces.map((w: Workspace) => ({
                  ...w,
                  records: w.records.map(r => normalizeRecord(r)),
                  archive: w.archive.map(r => normalizeRecord(r)),
                }))
                ...
                const oldArc = ((data.archive ?? []) as Array<Partial<ICURecord>>).map(r => normalizeRecord(r))
                const ws: Workspace = { id: generateId(), name: 'UTI', records: oldRec, archive: oldArc }
                localWs = [ws]
                localActiveId = ws.id
              }
              localStorage.setItem(STORAGE_KEYS.workspaces, JSON.stringify(localWs))
              localStorage.setItem(STORAGE_KEYS.activeWorkspace, localActiveId)
              localStorage.setItem('sea-icu-updated-at', supabaseUpdatedAt)
            }
          }
        } catch { /* Supabase indisponível — usa dados locais */ }
      }

      // Garante pelo menos um setor e activeId válido
      if (localWs.length === 0) {
        const ws: Workspace = { id: generateId(), name: 'UTI', records: [], archive: [] }
        localWs = [ws]
        localActiveId = ws.id
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="text-[8px] leading-none text-[#fca5a5]"
                >
                  ×
                </button>
              </div>
      ...
      <div className="chrome-board rounded-[1.8rem] p-3 md:p-6">
        ...
        {showSyncModal && (
          <div className="chrome-panel rounded-[1rem] p-2 space-y-1.5">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-[0.6rem] border border-white/10 bg-white/5 px-2.5 py-1.5 text-[8px] text-white/72 hover:bg-white/10 transition-colors"
                >
            ...
            <div className="space-y-1">
              <p className="text-[8px] text-white/48">Ou cole aqui o código de outro dispositivo para importar os dados dele:</p>
              <div className="flex items-center gap-2">
                <input
                  value={syncCodeInput}
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-[0.6rem] border border-[#60a5fa30] bg-[#60a5fa0a] px-2.5 py-1.5 text-[8px] font-semibold text-[#60a5fa] hover:bg-[#60a5fa18] transition-colors disabled:opacity-40"
                >
          ...
          <div className="space-y-1.5">
                ...
                {TAB_ITEMS.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex min-w-fit items-center gap-1.5 rounded-full px-3 py-1.5 text-[8px] font-semibold uppercase tracking-[0.14em] transition-all ${activeTab === tab.id ? 'bg-white/10 text-white' : 'text-white/48 hover:text-white/76'
                        }`}
                    >
                  ...
                  <div className="mt-2 space-y-1.5">
                    <div className="rounded-[0.7rem] border border-white/10 bg-white/[0.04] px-2 py-1">
                      <div className="scrollbar-hide flex flex-nowrap items-center gap-1.5 overflow-x-auto">
                        {calculations?.vtTargets?.length ? (
                          ...
                          <span className="text-[8px] text-white/46">Informe sexo e altura para calcular PBW e VC alvo.</span>
                        )}
                      </div>
                    </div>
                    ...
                    {calculations?.balance ? (
                      ...
                      >
                        ...
                        <p className="mt-0.5 text-[8px] leading-relaxed text-white/50">
            ...
            {activeTab === 'neuro' ? (
              <div className="space-y-1.5">
                ...
                <div className="chrome-panel rounded-[1rem] p-1.5 md:p-2">
                  ...
                  <div className="grid gap-1 grid-cols-5">
                    ...
                    <FieldShell label="RASS">
                      <select className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.rass} onChange={(e) => setField('rass', e.target.value)}>
                        <option value="">--</option>
                    ...
                    <FieldShell label="M.RASS">
                      <select className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.metaRASS} onChange={(e) => setField('metaRASS', e.target.value)}>
                        <option value="">--</option>
                        <option value="0">0 Alerta e calmo</option>
                        <option value="-1">-1 Sonolento</option>
                        <option value="-2">-2 Sedação leve</option>
                ...
                {/* Alertas Neuro Cruzados — Glasgow + Sedação + BNM */}
                {(() => {
                          ...
                          <div key={`sed-${index}`} className="rounded-[0.7rem] border border-white/10 bg-black/18 p-1.5" style={isSuspended ? { opacity: 0.6 } : undefined}>
                            <div className="grid gap-1 grid-cols-2 sm:grid-cols-[1.3fr_1fr_1fr_1fr_auto]">
                              ...
                              <FieldShell label="Dose Inicio">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.inicio} onChange={(event) => updateListItem('sedativos', index, 'inicio', event.target.value)} placeholder="ml/h" />
                              </FieldShell>
                              <FieldShell label="Dose Atual">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.atual} onChange={(event) => updateListItem('sedativos', index, 'atual', event.target.value)} placeholder="ml/h" disabled={isSuspended} />
                              </FieldShell>
                              <FieldShell label="Unidade">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.unidade} onChange={(event) => updateListItem('sedativos', index, 'unidade', event.target.value)} />
                              </FieldShell>
                        ...
                        currentRecord.bnmList.map((item, index) => {
                          ...
                          <div key={`bnm-${index}`} className="rounded-[0.7rem] border border-white/10 bg-black/18 p-1.5" style={isSuspended ? { opacity: 0.6 } : undefined}>
                            <div className="grid gap-1 grid-cols-2 sm:grid-cols-[1.3fr_1fr_1fr_1fr_auto]">
                              ...
                              <FieldShell label="Dose Inicio">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.inicio} onChange={(event) => updateListItem('bnmList', index, 'inicio', event.target.value)} placeholder="ml/h" />
                              </FieldShell>
                              <FieldShell label="Dose Atual">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.atual} onChange={(event) => updateListItem('bnmList', index, 'atual', event.target.value)} placeholder="ml/h" disabled={isSuspended} />
                              </FieldShell>
                              <FieldShell label="Unidade">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.unidade} onChange={(event) => updateListItem('bnmList', index, 'unidade', event.target.value)} />
                              </FieldShell>
                              <div className="flex items-end gap-1 pb-1">
                                {!isSuspended ? (
                        ...
                        <div key={`dva-${index}`} className="rounded-[0.7rem] border border-white/10 bg-black/18 p-1.5" style={isSuspended ? { opacity: 0.6 } : undefined}>
                          <div className="grid gap-1 grid-cols-2 md:grid-cols-[1.3fr_1fr_1fr_1fr_auto]">
                            ...
                            <FieldShell label="Inicio">
                              <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.inicio} onChange={(event) => updateListItem('dvaList', index, 'inicio', event.target.value)} placeholder="0.12" />
                            </FieldShell>
                            <FieldShell label="Dose atual">
                              <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.dose} onChange={(event) => updateListItem('dvaList', index, 'dose', event.target.value)} placeholder="0.08" disabled={isSuspended} />
                            </FieldShell>
                            <FieldShell label="Unidade">
                              <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={item.unidade} onChange={(event) => updateListItem('dvaList', index, 'unidade', event.target.value)} />
                            </FieldShell>
              ...
              <div className="space-y-1.5">
                  ...
                  <div className="grid gap-1.5 grid-cols-2 xl:grid-cols-4">
                    ...
                    {(currentRecord.tipoVia === 'RE-O2' || currentRecord.tipoVia === 'RE-MFS' || currentRecord.tipoVia === 'RE-MFR' || currentRecord.tipoVia === 'TQT-O2') && (
                      <FieldShell label="Fluxo O₂ (L/min)">
                        <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fluxoO2 ?? ''} onChange={(e) => setField('fluxoO2', e.target.value)} placeholder="3" />
                      </FieldShell>
                    )}
                    {currentRecord.tipoVia === 'RE-MFV' && (
                      <>
                        <FieldShell label="FiO₂ Venturi (%)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="40" />
                        </FieldShell>
                        <FieldShell label="Fluxo (L/min)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fluxoO2 ?? ''} onChange={(e) => setField('fluxoO2', e.target.value)} placeholder="8" />
                        </FieldShell>
                      </>
                    )}
                    {(currentRecord.tipoVia === 'RE-MFR') && (
                      <FieldShell label="FiO₂ (%)">
                        <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="60" />
                      </FieldShell>
                    )}
                    {currentRecord.tipoVia === 'HFNC' && (
                      <>
                        <FieldShell label="Fluxo HFNC (L/min)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fluxoO2 ?? ''} onChange={(e) => setField('fluxoO2', e.target.value)} placeholder="50" />
                        </FieldShell>
                        <FieldShell label="FiO₂ (%)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="40" />
                        </FieldShell>
                        <FieldShell label="Temp (°C)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.tempHFNC ?? ''} onChange={(e) => setField('tempHFNC', e.target.value)} placeholder="37" />
                        </FieldShell>
                      </>
                    )}
                    {currentRecord.tipoVia === 'TQT-AA' && (
                      <FieldShell label="Fluxo O₂ (L/min)">
                        <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fluxoO2 ?? ''} onChange={(e) => setField('fluxoO2', e.target.value)} placeholder="0" />
                      </FieldShell>
                    )}
                    ...
                    {(currentRecord.tipoVia === 'TOT' || currentRecord.tipoVia === 'TNT') && (
                      <>
                        ...
                        <FieldShell label="Data extubacao">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="dd/mm/aa" value={currentRecord.dataExtubacao} onChange={(event) => setField('dataExtubacao', event.target.value)} />
                        </FieldShell>
                        <FieldShell label="Hora extubacao">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="hh:mm" value={currentRecord.horaExtubacao} onChange={(event) => setField('horaExtubacao', event.target.value)} />
                        </FieldShell>
                        <FieldShell label="Data Re-IOT">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="dd/mm/aa" value={currentRecord.dataReIOT} onChange={(event) => setField('dataReIOT', event.target.value)} />
                        </FieldShell>
                        <FieldShell label="Hora Re-IOT">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="hh:mm" value={currentRecord.horaReIOT} onChange={(event) => setField('horaReIOT', event.target.value)} />
                        </FieldShell>
                      </>
                    )}
                    ...
                    {currentRecord.tipoVia.startsWith('TQT') && (
                      <>
                        ...
                        <FieldShell label="Data decanulacao">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="dd/mm/aa" value={currentRecord.dataDecanulacao} onChange={(event) => setField('dataDecanulacao', event.target.value)} />
                        </FieldShell>
                        <FieldShell label="Hora decanulacao">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="hh:mm" value={currentRecord.horaDecanulacao} onChange={(event) => setField('horaDecanulacao', event.target.value)} />
                        </FieldShell>
                        <FieldShell label="Data desc. VM">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="dd/mm/aa" value={currentRecord.dataDescVM} onChange={(event) => setField('dataDescVM', event.target.value)} />
                        </FieldShell>
                        <FieldShell label="Hora desc. VM">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="text" placeholder="hh:mm" value={currentRecord.horaDescVM} onChange={(event) => setField('horaDescVM', event.target.value)} />
                        </FieldShell>
                      </>
                    )}
                  </div>
                        ...
                        <div key={`${entry.ts}-${index}`} className="rounded-[1rem] border border-white/10 bg-black/18 p-3">
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-0.5 text-[8px] text-white/44">
                                pH {entry.pH || '--'} · PaCO2 {entry.paCO2 || '--'} · PaO2 {entry.paO2 || '--'} · HCO3 {entry.hco3 || '--'} · Lac {entry.lactato || '--'}
                              </p>
                              <p className="mt-0.5 text-[8px] text-white/36">
                                P/F {entry.pf || '--'} · S/F {entry.sf || '--'} · FiO2 {entry.fio2 || '--'}
                              </p>
                              {entry.obs ? <p className="mt-0.5 text-[8px] text-white/50">{entry.obs}</p> : null}
                            </div>
                ...
                <div className="chrome-panel rounded-[1rem] p-1.5 md:p-2">
                  ...
                  <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                    <FieldShell label="Modo ventilatorio" span="xl:col-span-2">
                      <select className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.modoVM} onChange={(event) => setField('modoVM', event.target.value)}>
                    ...
                    <div className="mt-2 space-y-1.5">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="VT (mL)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.vt} onChange={(e) => setField('vt', e.target.value)} placeholder="450" />
                        </FieldShell>
                        <FieldShell label="VC (mL)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.vc} onChange={(e) => setField('vc', e.target.value)} placeholder="420" />
                        </FieldShell>
                        <FieldShell label="VE (L/min)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ve} onChange={(e) => setField('ve', e.target.value)} placeholder={calculations?.minuteVentilation ? `${calculations.minuteVentilation.toFixed(1)} (auto)` : '8'} />
                        </FieldShell>
                        <FieldShell label="FR">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fr} onChange={(e) => setField('fr', e.target.value)} placeholder="18" />
                        </FieldShell>
                        <FieldShell label="PEEP">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.peep} onChange={(e) => setField('peep', e.target.value)} placeholder="10" />
                        </FieldShell>
                        <FieldShell label="FiO2">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="40" />
                        </FieldShell>
                      ...
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="Fluxo (L/min)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fluxo} onChange={(e) => setField('fluxo', e.target.value)} placeholder="60" />
                        </FieldShell>
                        <FieldShell label="Trigger">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.trigger} onChange={(e) => setField('trigger', e.target.value)} placeholder="-2" />
                        </FieldShell>
                        <FieldShell label="TI (s)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ti} onChange={(e) => setField('ti', e.target.value)} placeholder="1.0" />
                        </FieldShell>
                        <FieldShell label="I:E">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ie} onChange={(e) => setField('ie', e.target.value)} placeholder="1:2" />
                        </FieldShell>
                        <FieldShell label="P. Pico">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ppico} onChange={(e) => setField('ppico', e.target.value)} placeholder="28" />
                        </FieldShell>
                      ...
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="P. Mean">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.pmean} onChange={(e) => setField('pmean', e.target.value)} placeholder="12" />
                        </FieldShell>
                    ...
                    <div className="mt-2 space-y-1.5">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="PC (cmH2O)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ppico} onChange={(e) => setField('ppico', e.target.value)} placeholder="20" />
                        </FieldShell>
                        <FieldShell label="VC (mL)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.vc} onChange={(e) => setField('vc', e.target.value)} placeholder="420" />
                        </FieldShell>
                      ...
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        ...
                        <FieldShell label="TI (s)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ti} onChange={(e) => setField('ti', e.target.value)} placeholder="0.8" />
                        </FieldShell>
                        <FieldShell label="I:E">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ie} onChange={(e) => setField('ie', e.target.value)} placeholder="1:2" />
                        </FieldShell>
                        <FieldShell label="P. Plato">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.pplato} onChange={(e) => setField('pplato', e.target.value)} placeholder="18" />
                        </FieldShell>
                        <FieldShell label="P. Mean">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.pmean} onChange={(e) => setField('pmean', e.target.value)} placeholder="12" />
                        </FieldShell>
                        <FieldShell label="Rampa (s)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.rampa ?? ''} onChange={(e) => setField('rampa', e.target.value)} placeholder="0.15" />
                        </FieldShell>
                        <FieldShell label="Ciclagem (%)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ciclagem} onChange={(e) => setField('ciclagem', e.target.value)} placeholder="25" />
                        </FieldShell>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="mt-2 space-y-1.5">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="PS (cmH2O)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ps} onChange={(e) => setField('ps', e.target.value)} placeholder="10" />
                        </FieldShell>
                        <FieldShell label="VC (mL)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.vc} onChange={(e) => setField('vc', e.target.value)} placeholder="420" />
                        </FieldShell>
                        ...
                        <FieldShell label="PEEP">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.peep} onChange={(e) => setField('peep', e.target.value)} placeholder="8" />
                        </FieldShell>
                        <FieldShell label="FiO2">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="40" />
                        </FieldShell>
                      ...
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        ...
                        <FieldShell label="I:E">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ie} onChange={(e) => setField('ie', e.target.value)} placeholder="1:3" />
                        </FieldShell>
                        <FieldShell label="P0.1">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.p01} onChange={(e) => setField('p01', e.target.value)} placeholder="2.2" />
                        </FieldShell>
                    ...
                    <div className="mt-2">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        ...
                        <FieldShell label="Interface">
                          <select className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.interfaceVNI} onChange={(e) => setField('interfaceVNI', e.target.value)}>
                    ...
                    <div className="mt-2">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="IPAP">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ipap} onChange={(e) => setField('ipap', e.target.value)} placeholder="15" />
                        </FieldShell>
                        <FieldShell label="EPAP">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.epap} onChange={(e) => setField('epap', e.target.value)} placeholder="8" />
                        </FieldShell>
                        <FieldShell label="FiO2">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="40" />
                        </FieldShell>
                    ...
                    <div className="mt-2 space-y-1.5">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-5">
                        ...
                        <FieldShell label="T-High (s)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ti} onChange={(e) => setField('ti', e.target.value)} placeholder="4.5" />
                        </FieldShell>
                        <FieldShell label="P-Low (cmH2O)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.peep} onChange={(e) => setField('peep', e.target.value)} placeholder="0" />
                        </FieldShell>
                        <FieldShell label="T-Low (s)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.trigger} onChange={(e) => setField('trigger', e.target.value)} placeholder="0.5" />
                        </FieldShell>
                        <FieldShell label="FiO2">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="60" />
                        </FieldShell>
                    ...
                    <div className="mt-2 space-y-1.5">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-5">
                        ...
                        <FieldShell label="% Suporte">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ps} onChange={(e) => setField('ps', e.target.value)} placeholder="70" />
                        </FieldShell>
                        <FieldShell label="Trigger">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.trigger} onChange={(e) => setField('trigger', e.target.value)} placeholder="-2" />
                        </FieldShell>
                    ...
                    <div className="mt-2 space-y-1.5">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="PS / NAVA level">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ps} onChange={(e) => setField('ps', e.target.value)} placeholder="2.0" />
                        </FieldShell>
                        <FieldShell label="VC (mL)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.vc} onChange={(e) => setField('vc', e.target.value)} placeholder="420" />
                        </FieldShell>
                    ...
                    <div className="mt-2 space-y-1.5">
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-5">
                        <FieldShell label="mPaw (cmH2O)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ppico} onChange={(e) => setField('ppico', e.target.value)} placeholder="30" />
                        </FieldShell>
                        <FieldShell label="FiO2">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.fio2} onChange={(e) => setField('fio2', e.target.value)} placeholder="60" />
                        </FieldShell>
                        <FieldShell label="ΔP amplitude">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.pplato} onChange={(e) => setField('pplato', e.target.value)} placeholder="60" />
                        </FieldShell>
                        <FieldShell label="Hz">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.hfovHz ?? ''} onChange={(e) => setField('hfovHz', e.target.value)} placeholder="5" />
                        </FieldShell>
                        <FieldShell label="TI (%)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.ti} onChange={(e) => setField('ti', e.target.value)} placeholder="33" />
                        </FieldShell>
                      </div>
                      <div className="grid gap-1 grid-cols-3 xl:grid-cols-6">
                        <FieldShell label="I:E">
                        ...
                        <FieldShell label="Trigger">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.trigger} onChange={(e) => setField('trigger', e.target.value)} placeholder="off" />
                        </FieldShell>
                        <FieldShell label="Bias Flow (L/min)">
                          <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.hfovBiasFlow ?? ''} onChange={(e) => setField('hfovBiasFlow', e.target.value)} placeholder="40" />
                        </FieldShell>
                        <FieldShell label="Observacoes" span="xl:col-span-3">
                          <AutoGrowTextarea value={currentRecord.vmObs ?? ''} onChange={(v) => setField('vmObs', v)} placeholder="Obs..." />
                        </FieldShell>
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-1 flex flex-wrap gap-x-2 gap-y-0.5">
                                  {params.map((p, pi) => (
                                    <span key={pi} className="text-[8px] text-white/50">
                                ...
                                {calcs.length > 0 && (
                                  <div className="mt-1 flex flex-wrap gap-x-2 gap-y-0.5">
                                    {calcs.map((c, ci) => (
                                      <span key={ci} className="text-[8px]" style={{ color: c.color || 'rgba(255,255,255,0.36)' }}>
                        ...
                        {peepRows.map((row, index) => (
                          <div key={`peep-row-${index}`} className="rounded-[0.7rem] border border-white/10 bg-black/18 p-1.5">
                            ...
                            <div className="grid gap-1 grid-cols-3">
                              <FieldShell label="PEEP">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="number" value={row.peep} onChange={(event) => setPeepOptField(index, 'peep', event.target.value)} placeholder="10" />
                              </FieldShell>
                              <FieldShell label="Plato">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="number" value={row.plato} onChange={(event) => setPeepOptField(index, 'plato', event.target.value)} placeholder="22" />
                              </FieldShell>
                              <FieldShell label="SI">
                                <input className={INPUT_CLASS_SM} style={INPUT_STYLE} value={row.si} onChange={(event) => setPeepOptField(index, 'si', event.target.value)} placeholder="=1" />
                              </FieldShell>
                            </div>
                          </div>
                        ))}
                      </div>
                      <div className="mt-1.5 grid gap-1 grid-cols-2">
                        <MetricChip
                ...
                <div className="chrome-panel rounded-[1rem] p-1.5 md:p-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      <p className="text-[7px] font-semibold uppercase tracking-[0.14em] text-white/40">Calculadora de desmame</p>
                    ...
                    <>
                      ...
                      <div className="mt-1 flex items-center gap-1">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const el = document.getElementById('__desmame_info__')
                ...
                {(() => {
                  ...
                  // 1. Causa reversível — toggle manual
                  const causaRev = currentRecord.weanCausaReversivel === 'sim'
                  eligibility.push({ label: 'Causa reversivel', met: currentRecord.weanCausaReversivel ? causaRev : null, detail: causaRev ? 'Marcada como reversivel' : currentRecord.weanCausaReversivel ? 'Nao reversivel' : 'Avaliar clinicamente' })
                  ...
                  // 3. Neurológico — Glasgow >= 8 OU toggle manual
                  const glasgowVal = typeof calculations?.glasgow?.total === 'number' ? calculations.glasgow.total : 0
                  const neuroManual = currentRecord.weanNeuroOk === 'sim'
                  const neuroOk = neuroManual || glasgowVal >= 8
                  eligibility.push({ label: 'Neurologico', met: (glasgowVal > 0 || currentRecord.weanNeuroOk) ? neuroOk : null, detail: neuroManual ? 'Marcado adequado' : glasgowVal > 0 ? `Glasgow ${glasgowVal}${neuroOk ? '' : ' — rebaixado'}` : 'Nao avaliado' })
                  ...
                  // 10. Tosse / proteção VA — tosse eficaz + PFT >= 60
                  const tosseOk = currentRecord.weanTosse === 'sim'
                  const pftVal = currentRecord.weanPeakFlow ? parseFloat(currentRecord.weanPeakFlow) : 0
                  const pftOk = pftVal >= 60
                  const glasgowSuf = glasgowVal >= 9
                  const vaOk = tosseOk || (pftVal > 0 && pftOk) || glasgowSuf
                  const vaDetail = [tosseOk ? 'Tosse eficaz' : '', pftVal > 0 ? `PFT ${pftVal}${pftOk ? '' : ' (<60)'}` : '', glasgowVal > 0 ? `Glasgow ${glasgowVal}` : ''].filter(Boolean).join(' · ') || 'Nao avaliado'
                  eligibility.push({ label: 'Protecao de VA', met: (currentRecord.weanTosse || pftVal > 0 || glasgowVal > 0) ? vaOk : null, detail: vaDetail })
                  ...
                  return (
                    <div className="chrome-panel rounded-[1rem] p-1.5 md:p-2">
                      ...
                      <div className="mb-3 rounded-[1rem] border border-white/8 bg-black/20 p-2.5">
                        ...
                        {allMet && (
                          <p className="mt-2 text-center text-[8px] font-semibold text-[#4ade80]">✓ Todos os criterios atendidos — elegivel para TRE</p>
                        )}
                      </div>
                      ...
                      <div className="mb-3 rounded-[0.8rem] border border-white/6 bg-black/15 p-2 space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {[
                            ...
                            { field: 'weanNeuroOk', label: 'Neuro OK' },
                            ...
                            <button key={item.field} type="button"
                              ...
                              className="rounded-full border px-2 py-0.5 text-[8px] font-semibold transition"
                          ...
                          <input className="w-16 rounded-full border border-white/8 bg-white/[0.02] px-2 py-0.5 text-[8px] text-white/70 outline-none placeholder:text-white/20" value={currentRecord.weanPeakFlow} onChange={(e) => setField('weanPeakFlow', e.target.value)} placeholder="PFT" />
                        </div>

                        <div className="flex flex-wrap items-center gap-1">
                          <span className="text-[7px] text-white/25">CLT:</span>
                          <button type="button" onClick={() => setField('weanCuffLeak', currentRecord.weanCuffLeak === 'negativo' ? '' : 'negativo')}
                            className="rounded-full border px-2 py-0.5 text-[7px] font-semibold transition"
                            style={currentRecord.weanCuffLeak === 'negativo' ? { background: 'rgba(74,222,128,0.1)', borderColor: 'rgba(74,222,128,0.3)', color: '#4ade80' } : { background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.30)' }}
                          >Neg (vazamento)</button>
                          <button type="button" onClick={() => setField('weanCuffLeak', currentRecord.weanCuffLeak === 'positivo' ? '' : 'positivo')}
                            className="rounded-full border px-2 py-0.5 text-[7px] font-semibold transition"
                            style={currentRecord.weanCuffLeak === 'positivo' ? { background: 'rgba(248,113,113,0.1)', borderColor: 'rgba(248,113,113,0.3)', color: '#f87171' } : { background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.30)' }}
                          >Pos (sem vazamento)</button>
                          <input className="w-16 rounded-full border border-white/8 bg-white/[0.02] px-2 py-0.5 text-[7px] text-white/70 outline-none placeholder:text-white/20" value={currentRecord.weanCuffLeakDiff} onChange={(e) => setField('weanCuffLeakDiff', e.target.value)} placeholder="Diff mL" />
                          <span className="text-[7px] text-white/25">TRE:</span>
                          <select className="rounded-full border border-white/8 bg-white/[0.02] px-1.5 py-0.5 text-[7px] text-white/60 outline-none" value={currentRecord.weanTRETipo} onChange={(event) => setField('weanTRETipo', event.target.value)}>
                            ...
                            <div key={p} className="flex items-center gap-1.5">
                              {isTREPhase ? (
                                <button
                                  ...
                                  className="rounded-[0.5rem] border px-2 py-0.5 text-[8px] font-semibold transition-all cursor-pointer"
                                  style={col
                                ...
                                <span
                                  className="rounded-[0.5rem] border px-2 py-0.5 text-[8px] font-semibold transition-all"
                                  style={col
                                    ? { color: col, borderColor: `${col}40`, background: `${col}12` }
                                    : { color: 'rgba(255,255,255,0.28)', borderColor: 'rgba(255,255,255,0.10)', background: 'rgba(255,255,255,0.03)' }
                                  }
                      ...
                      <div className="flex flex-wrap justify-center gap-2 mb-3">
                        {(isTOT || wasOnVM) && (
                          <button type="button" onClick={() => setField('extOK', extActive ? '' : '1')}
                            className="rounded-[0.8rem] border px-3 py-1.5 text-[8px] font-semibold"
                      ...
                      {treActive && (
                        <div className="mb-3 grid gap-2 grid-cols-2">
                          <FieldShell label="Data TRE">
                            <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="date" value={currentRecord.treDt} onChange={(e) => setField('treDt', e.target.value)} />
                          </FieldShell>
                          <FieldShell label="Hora TRE">
                            <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="time" value={currentRecord.treTm} onChange={(e) => setField('treTm', e.target.value)} />
                          </FieldShell>
                        </div>
                      )}
                          ...
                          <div className="flex gap-2">
                            {(['Sucesso', 'Falha'] as const).map((opt) => (
                              <button key={opt} type="button" onClick={() => setField('extResult', currentRecord.extResult === opt ? '' : opt)}
                                className="rounded-[0.7rem] border px-3 py-1 text-[8px] font-semibold"
                    ...
                    {PROTOCOL_OPTIONS.map((protocol) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-[0.6rem] border px-2 py-1 text-left transition-all"
                          style={{
                        ...
                        >
                          ...
                          <span className="text-[8px] font-medium text-white/76">{protocol.label}</span>
                        </button>
                      )
                    })}
                  </div>
                        ...
                        return (
                          <div key={pid}>
                            ...
                            >
                              ...
                              <span className="text-[8px] font-bold" style={{ color: proto.color }}>{proto.label}</span>
                            </button>
                            {isOpen ? (
                ...
                <div className="chrome-panel rounded-[1rem] p-1.5 md:p-2">
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-2"
                  >
                    <p className="text-[7px] font-semibold uppercase tracking-[0.14em] text-white/40">Prona / recrutabilidade</p>
                      ...
                      {proneActive && (
                        <div className="mt-2 grid gap-2 grid-cols-3">
                          <FieldShell label="Tempo prona">
                            <select className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.pronaTempo} onChange={(event) => setField('pronaTempo', event.target.value)}>
                          ...
                          <FieldShell label="Data inicio">
                            <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="date" value={currentRecord.pronaData} onChange={(event) => setField('pronaData', event.target.value)} />
                          </FieldShell>
                          <FieldShell label="Hora inicio">
                            <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="time" value={currentRecord.pronaHora} onChange={(event) => setField('pronaHora', event.target.value)} />
                          </FieldShell>
                        </div>
                      )}
                      ...
                      <div className="mt-2 border-t border-white/8 pt-2">
                        ...
                        <div className="grid gap-1 grid-cols-2 xl:grid-cols-4">
                          ...
                          <FieldShell label="Vol. exp. (mL)">
                            <input className={INPUT_CLASS_SM} style={INPUT_STYLE} type="number" value={currentRecord.recVolExp} onChange={(event) => setField('recVolExp', event.target.value)} placeholder="600" />
                          </FieldShell>
                          <MetricChip
                            label="Diferenca"
              ...
              <div className="space-y-1.5">
                  ...
                  <div className="grid gap-2 grid-cols-2 xl:grid-cols-4">
                    {PERME_ITEMS.map((item) => (
                      <FieldShell key={item.key} label={item.label}>
                        <select className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord[item.key]} onChange={(event) => setField(item.key, event.target.value)}>
                ...
                <div className="chrome-panel rounded-[1rem] p-1.5 md:p-2">
                  ...
                  <FieldShell label="Score IMS">
                    <select className={INPUT_CLASS_SM} style={INPUT_STYLE} value={currentRecord.imsScore} onChange={(event) => setField('imsScore', event.target.value)}>
              ...
              >
                ...
                >
                  ...
                  <input
                    ...
                    onKeyDown={e => {
                      if (e.key === 'Enter') editingWsId ? renameWorkspace(editingWsId, wsNameInput) : createWorkspace(wsNameInput)
                    }}
                    placeholder="Ex: UTI 2, Emergência, Enfermaria..."
                  ...
                  <div className="mt-3 flex gap-2">
                    {editingWsId && workspaces.length > 1 && (
                      <button
                        ...
                        className="flex-1 rounded-[0.7rem] border border-red-500/20 bg-red-500/8 py-1.5 text-[8px] text-red-400/80 hover:bg-red-500/14"
                      >
                        Excluir setor
                      </button>
                    )}
                    <button
                      onClick={() => editingWsId ? renameWorkspace(editingWsId, wsNameInput) : createWorkspace(wsNameInput)}
                      disabled={!wsNameInput.trim()}
                      className="flex-1 rounded-[0.7rem] border border-white/12 bg-white/[0.06] py-1.5 text-[8px] text-white/70 disabled:opacity-40"
                    >
